...
async def eliminar_localitzacio(id: int):
    ...
    # Verificar si hay caminos usando esta localización
    check_query = """
    ...
    if result['count'] > 0:
        # Si hay caminos usando esta localización, redirigir con error
...
async def list_camins(request: Request):
    """Lista todos los caminos con sus localizaciones"""
    query = """
        SELECT 
            c.id,
            c.localitzacio1,
...
async def nou_cami_form(request: Request):
    """Formulario para crear nuevo camino"""
...
async def crear_cami(
    ...
    query = """
        INSERT INTO camins (localitzacio1, localitzacio2) 
        VALUES (%s, %s)
    """
